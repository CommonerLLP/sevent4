...
- **Do not add a `Co-Authored-By` / AI-attribution trailer to commits** — this is an
  org-wide rule for CommonerLLP repos.

## Quality checks (run before opening a PR)

```bash
python3 -m unittest discover -s tests       # the full suite must pass (currently 93 tests)
...
`data/`, `notes/`, `docs/`, `memory/`, and `WORKING.md` are **local-only** working
material and are gitignored. Do not commit secrets, personal email addresses, internal
strategy notes, or large raw datasets. The pre-commit hook runs a leak check; do not
