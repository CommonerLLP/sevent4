...
The Unelected City should be automation-friendly but not
automation-dependent.

Automation may run acquisition, extraction, build, test, and browser QA steps,
but the architecture must be deterministic enough that a human can reproduce
the result from source profiles, manifests, code, and tests.

The design rule is conservative:

```text
Use the simplest deterministic workflow that can prove the claim.
Escalate to assisted workflows only when the work needs judgment, exploration,
or multi-step adaptation.
Keep durable proof in code, source manifests, claim contracts, tests, run logs,
browser QA, and PR review.
```

Automation rules:

- Do not bypass source profiles and fact schemas for one-off prose.
- Do not build generic infrastructure in The Unelected City when it belongs in
  `commoner-probe`, `partial-recall`, or `budget-crawler`.
- Expose missing capabilities instead of silently working around them.
- Checkpoints are not Git history. Durable review happens through commits,
  PRs, tests, run logs, and browser QA artifacts.
- Remote execution needs explicit environment definitions, secrets policy,
  outbound-network policy, hooks, logs, and reviewable artifacts.
- Use named branches and PRs.
- Run tests and browser QA for public surfaces.
- Do not publish internal file paths or private analytical state in public PR
  bodies.
