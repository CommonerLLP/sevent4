...
community libraries), and all 35 are now located — 20 at verified or rooftop precision
(DPL-published coordinates and Google rooftop matches) and 15 at approximate
address-level precision. Delhi's Metro network is joined from OpenStreetMap (246
stations and the line geometry), and ward boundaries are the pre-2022 set (the 2022
unified-MCD 250-ward delimitation is not openly available as geometry). This is enough
to compute ward walk-access and transit-siting for the fixed network, which we do
below; the maps should be read as indicative of sparsity, not as a survey, and mobile
service points are excluded because they are not fixed access. Full DTC/cluster-bus
route geometry remains the next step.
...
**59 of 290 wards — about 20 percent — fall within a 1.2 km (roughly 15-minute) walk
of a fixed DPL branch**, and the median ward sits **2.13 km** from its nearest one. The
branches cluster in the old core (New Delhi, Central, North, and West districts hold
two-thirds of them) while the outer districts — South West, North East, East — have one
or two each. For most of Delhi, the nearest public library is not walkable.

![Distance from each Delhi ward to its nearest fixed DPL branch. Only about 20 percent of wards lie within a 1.2 km walk; the periphery is largely unserved. Coordinates: 20 verified/rooftop, 15 approximate; mobile service points excluded.](figures/figD3_dpl_walk_access.png){#fig-walk width=100%}

Nor does transit rescue the reach. Joining the branches to Delhi's Metro — one of the
densest rapid-transit networks in the country — only **17 of the 35 fixed branches sit
within 800 m of a Metro station, and just 9 within 400 m** (Figure 4). Half the fixed
...
The maps are indicative, not survey-grade: 15 of the 35 coordinates are approximate
(address-level rather than rooftop) and the ward layer is the pre-2022 set. But the
pattern is robust to that noise — a sparse network, concentrated in the core, only
...
are: completing the spatial picture — geocoding the mobile service points, upgrading
the 15 approximate fixed coordinates to rooftop precision, and adding DTC/cluster-bus
route geometry to the Metro, RRTS, and suburban rail already joined; and extending the
disclosed finance series back before 2021-22 to date the onset of under-spending.
