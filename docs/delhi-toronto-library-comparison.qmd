...
fifteen years. The Toronto Public Library appears only as a background yardstick.
...
on establishment rather than books, Delhi's central library does not spend at all.
...
One limitation is structural and stated up front: of DPL's 111 published locations,
only five are geocoded so far, and Delhi's transit layers (DMRC Metro, DTC and cluster
buses) are not yet joined. We can report service-point *density* now, but the
ward-level walk-access and transit-siting maps built for Ahmedabad are not yet
computable for Delhi and are flagged as the next step.
...
The full reach question — walk time and transit access by ward — cannot yet be
answered for Delhi: only five of 111 published DPL locations are geocoded, and the
Metro and bus layers are not yet joined. What can be said now is about density. On a
conservative count DPL operates about 40 service points (fixed libraries plus mobile
buses) for 19 million residents — roughly 2.1 per million, against Toronto's 34 per
million, a sixteen-fold difference before any travel-time disadvantage is measured.
Even on the most inclusive count of DPL touchpoints (about 163, including mobile
service points and book-reading centres) the system is thin relative to the population
it nominally serves. The next step is to geocode the full location list and join
Delhi's Metro and bus networks, reproducing for Delhi the ward walk-access and
transit-siting maps already built for Ahmedabad.
...
functioning system's thirty-plus — and its locations are not yet even mapped. The
immediate, low-cost step is to geocode the full network and place service on Delhi's
Metro and bus corridors, which already carry the residents DPL is meant to serve.
...
are: geocoding all 111 locations and joining the DMRC Metro and DTC/cluster-bus layers
to build the ward walk-access and transit-siting maps; and extending the disclosed
finance series back before 2021-22 to date the onset of under-spending.

## Conclusion

The Delhi Public Library fails the tests a city library should pass. It is sparse —
about two service points per million residents — and its network is not yet even
mapped. Its public use has collapsed: issues down about 77 percent over a decade, new
