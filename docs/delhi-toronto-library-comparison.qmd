...
the network barely reaches: with all 35 fixed branches now located, only about 20 percent
...
The spatial layers are now built, with their limits stated up front. DPL is not a
flat branch network. Its 111 published locations break into 35 fixed physical service
locations — 1 headquarters, 4 zonal libraries, 8 branch-like fixed libraries, 1
special fixed Braille library, 20 sub-branches, and 1 community library — plus 76
scheduled mobile-library service points. For internal modelling all 111 now have
coordinates. The public-provenance coordinate layer is narrower: 33 locations come
from source coordinates or DPL-published map pins, while 78 locations use Google
geocoding and are flagged as internal-use until independently verified. Among the 35
fixed physical locations, 33 have source/DPL-map provenance and 2 are Google-geocoded.
The same audit records what DPL does not disclose in a branch-wise machine-readable
form: max seating capacity, opening hours, weekly open hours, branch collection size,
and collection types are blank for all 111 locations in the current public extract.
Delhi's Metro is joined from OpenStreetMap (246 stations and the line geometry) and
the DTC/cluster bus network from the Open Transit Data GTFS feed (10,559 stops;
2,400-plus routes); ward boundaries are the pre-2022 set (the 2022 unified-MCD
250-ward delimitation is not openly available as geometry). This is enough to compute
ward walk-access and transit-siting for the fixed network, which we do below; the maps
should be read as indicative of sparsity, not a survey, and mobile service points are
excluded because they are scheduled stops rather than permanent public rooms.
...
Density understates the problem, because the fixed physical locations are not spread to where
people live. Mapping all 35 fixed DPL physical locations against Delhi's wards (Figure 3), only
**57 of 290 wards — about 20 percent — fall within a 1.2 km (roughly 15-minute) walk
of a fixed DPL location**, and the median ward sits **2.53 km** from its nearest one; put
spatially, barely **9 percent of the city's land area** lies within that walk. The
network is also lopsided: its mean centre sits about **7 km east-northeast of Delhi's
geographic centre**, pulled into the old core (New Delhi, North, Central, and Shahdara
districts hold many fixed locations) while outer districts — South West, East, North East —
have one each. For most of Delhi, the nearest public library is not walkable.

![Distance from each Delhi ward to its nearest fixed DPL physical location. Only about 20 percent of wards lie within a 1.2 km walk and barely 9 percent of the city's land area; the periphery is largely unserved. Coordinates: 33 fixed locations from source or DPL-published map provenance, 2 fixed locations from Google geocoding; mobile service points excluded.](figures/figD3_dpl_walk_access.png){#fig-walk width=100%}

Transit only partly rescues the reach, and unevenly by mode. Against Delhi's Metro —
one of the densest rapid-transit networks in the country — only **16 of the 35 fixed
DPL physical locations sit within 800 m of a Metro station, and just 7 within 400 m** (Figure 4):
half the network is off the *rapid*-transit grid. The DTC and cluster bus network does
markedly better — **27 of the 35 fixed locations are within 400 m of a bus stop** (of 10,559
stops citywide) — so most branches are reachable by bus, the slower and more crowded
mode, while the faster Metro that could widen their catchment largely misses them.
Siting new service on the Metro corridors where Delhi already moves is the most direct
way to extend reach without building a single new branch.

![Fixed DPL branches against Delhi's full transit network — Metro lines and stations (OpenStreetMap) over the DTC/cluster bus-stop web (Open Transit Data GTFS, 10,559 stops). Green = within 800 m of a Metro station; amber = bus-served only (within 400 m of a bus stop but off the Metro); red = beyond 400 m of any transit. Half the network is off the rapid-transit grid, but the bus reaches almost all of it.](figures/figD4_dpl_transit_siting.png){#fig-transit width=100%}

Two caveats bound these maps. First, the fixed-location coordinates are strong enough
for internal modelling but not all equally publishable as an open coordinate layer:
33 of the 35 fixed locations come from source/DPL-map provenance, while Karol Bagh
Closed and Mundka Closed currently rely on Google geocoding. The residual uncertainty
therefore includes both those two Google-assisted points and the ward layer (the
pre-2022 set). Second, and more important, the 1.2 km walk is a deliberately
strict floor. Many Delhi residents do not reach a fixed library on foot: they take an
auto-rickshaw, an app-cab (Ola/Uber), a two-wheeler, a cycle-rickshaw, or a bicycle,
and this pedestrian measure counts none of those modes — so it understates who *could*
in principle reach a library, and the Metro cut (Figure 4) captures only one slice of
that wider, motorised catchment. The walk standard is kept all the same because it is
the right test for the readers a public library most exists to serve — children,
students, older people, and the carless poor, for whom the short walk is the only free
mode. The pattern is robust to both caveats: a sparse network, concentrated in the
core, only loosely tied to the networks that move the rest of the city.
...
functioning system's thirty-plus — and the mapped branches reach only about 20 percent
...
Delhi's fragmented NCT/MCD/NDMC arrangement, and its own RTI manifest points to the
Delhi Library Board as a governing body whose functions and powers need to be read
alongside the Director General and Ministry of Culture grant chain. Accountability for
decline is diffuse by design. Saying who is responsible for use, renewal, spending,
and disclosure is the precondition for fixing any of it.
...
access points, and branch-level hours and capacity. The branch-detail gap is explicit
in the data: the current public extract has 0 of 111 locations with max seating
capacity, 0 of 111 with opening hours or weekly open hours, 0 of 111 with branch
collection size, and 0 of 111 with collection-type detail. Delhi therefore cannot yet
be compared to Ahmedabad on a simple "all branches open office hours" basis without a
better DPL source, a request to the Director General/Delhi Library Board, or an RTI.
The two highest-value extensions are: extending the disclosed finance series back
before 2021-22 to date the onset of under-spending; and adding the per-branch physical
detail DPL does not publish in one place — reading-room capacity, opening hours,
staff, internet access, collection size, and collection type — which a short RTI or
site survey could supply to turn the reach maps into a service audit.
...
the old core: only about 20 percent of wards are within a short walk of a fixed
