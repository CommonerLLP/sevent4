...
def _heat_panel_js() -> str:
    """Sidebar 'Whose neighbourhood is the oven?' card. Fetches this city's runtime
    heat summary (layers/ward_heat_summary.json) and names its hottest wards, turned
    to the thesis. Fully data-driven — a heat refresh rewrites the summary and the
    strip updates without rebuilding the console. Cities with a verified city-wide
    CSE figure (WHY/heat roster) get the extra headline prepended."""
    return """
  (function(){
    var box=document.getElementById('heatbox'), host=document.getElementById('heatpanel');
    if(!box||!host) return;
    fetch('layers/ward_heat_summary.json').then(function(r){return r.json();}).then(function(s){
...
def _js() -> str:
  ...
  function officialRowHtml(meta, props) {
    ...
    if (!rec) return "";
    if (rec.name) {
      // a few acquired records cite more than one URL separated by " ; ", or a
      // non-URL provenance note (e.g. "reused from this repo's own <file>") —
      // only linkify an actual http(s) URL, same discipline as the /officials
      // sub-page, never a bare non-URL string
      const firstSource = (rec.source || "").split(" ; ")[0].trim();
      const isUrl = firstSource.startsWith("http://") || firstSource.startsWith("https://");
      const src = isUrl
        ? ` <a class="pfsrc" href="${escapeHtml(firstSource)}" target="_blank" rel="noopener">source</a>`
        : (firstSource ? ` <span class="pfsrc-text">${escapeHtml(firstSource)}</span>` : "");
