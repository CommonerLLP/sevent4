...
def fig_decline(rows: list[dict] | None = None, stats: dict | None = None):
    """Hero: DPL issues (circulation) and total members, 2009-10 to 2023-24."""
    df = _frame(rows)
    stats = stats or decline_stats(df.to_dict("records"))
    yr = df["year"].tolist()
    x = range(len(yr))
...
def fig_finance(rows: list[dict] | None = None, stats: dict | None = None):
    """Grant received vs spent vs unspent — DPL returns/under-spends its grant."""
    df = _frame(rows)
    stats = stats or finance_stats(df.to_dict("records"))
...
render_decline = fig_decline
render_finance = fig_finance
