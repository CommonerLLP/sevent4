"""Pure Delhi Public Library service-hierarchy logic: service-tier classification,
physical-access model, coordinate-provenance grouping (incl. the Google-derived
public-dataset policy), per-row detail building, and the summary. No filesystem IO.
"""
...
DETAIL_FIELDS = [
    "library_id", "name", "service_tier", "source_location_type", "hierarchy_rank",
    "physical_access_model", "zone", "address", "latitude", "longitude", "coordinate_status",
    "coordinate_provenance_group", "geocode_confidence", "geocode_provider",
    "max_seating_capacity", "opening_hours", "weekly_open_hours", "branch_collection_size",
    "collection_types", "branch_detail_status", "usable_for_internal_access_model",
    "usable_for_public_open_dataset", "notes",
]

SUMMARY_FIELDS = [
    "total_locations", "fixed_physical_locations", "mobile_service_points", "headquarters",
    "zonal_libraries", "branch_libraries", "special_fixed_libraries", "sub_branch_libraries",
    "community_libraries", "internal_geocoded_locations", "public_provenance_coordinates",
    "google_geocode_coordinates", "branchwise_seating_capacity_locations",
    "branchwise_opening_hours_locations", "branchwise_collection_size_locations",
    "branchwise_collection_type_locations", "access_model_status", "notes",
]

TIER_RANK = {
    "headquarters": 10, "zonal_library": 20, "branch_library": 30, "special_fixed_library": 35,
    "sub_branch_library": 40, "community_library": 50, "mobile_service_point": 60, "other": 90,
}
...
def hierarchy_rows(source_rows: list[dict[str, str]], geocoded_rows: list[dict[str, str]]) -> list[dict[str, str]]:
    ...
    for source in source_rows:
        ...
        rows.append({
            "library_id": source.get("library_id", ""), "name": source.get("name", ""),
            "service_tier": tier, "source_location_type": source.get("location_type", ""),
            "hierarchy_rank": str(TIER_RANK.get(tier, TIER_RANK["other"])),
            "physical_access_model": physical_access_model(tier), "zone": source.get("zone", ""),
            "address": source.get("address", ""), "latitude": lat, "longitude": lon,
            "coordinate_status": "located" if lat and lon else "missing_coordinates",
            "coordinate_provenance_group": provenance, "geocode_confidence": geocode_confidence,
            "geocode_provider": geocode_provider, "max_seating_capacity": "", "opening_hours": "",
            "weekly_open_hours": "", "branch_collection_size": "", "collection_types": "",
            "branch_detail_status": "missing_branchwise_public_detail",
            "usable_for_internal_access_model": internal_ok, "usable_for_public_open_dataset": public_ok,
...
def summarize_hierarchy(rows: list[dict[str, str]]) -> dict[str, str]:
    ...
    return {
        "total_locations": str(len(rows)), "fixed_physical_locations": str(fixed),
        "mobile_service_points": str(tiers["mobile_service_point"]), "headquarters": str(tiers["headquarters"]),
        "zonal_libraries": str(tiers["zonal_library"]), "branch_libraries": str(tiers["branch_library"]),
