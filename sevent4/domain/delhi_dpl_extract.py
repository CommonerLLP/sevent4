"""Pure Delhi Public Library extraction logic: HTML/text parsing of zone and
mobile location pages, annual-report metric parsing, the bounded per-capita time
series, and the latest-year long-metrics table. No filesystem IO — callers pass
already-read HTML/text strings and parsed rows.
"""
...
MANIFEST_FIELDS = [
    "kind", "text", "url", "local_path", "status", "bytes", "sha256",
    "valid_pdf", "repo_storage", "notes",
]

ANNUAL_FIELDS = [
    "year", "source_file", "source_url",
    "adult_members", "child_members", "total_members",
    "adult_issues", "child_issues", "total_issues",
    "reading_room_attendance", "collection_total", "books_added_to_stock",
    "opening_unspent_rs", "grant_received_rs", "other_income_rs", "total_available_rs",
    "returned_to_ministry_rs", "total_expenditure_rs", "closing_unspent_rs",
    "confidence", "notes",
]

TIME_SERIES_FIELDS = [
    "year", "fiscal_start_year", "fiscal_end_year", "observation_status", "source_url",
    "total_members", "total_issues", "reading_room_attendance", "collection_total",
    "books_added_to_stock", "total_expenditure_rs", "population_denominator",
    "members_per_1m", "issues_per_1m", "reading_room_attendance_per_1m",
    "collection_refresh_pct", "issues_per_collection_item", "notes",
]

METRICS_LONG_FIELDS = [
    "library_system", "place", "year", "metric_group", "metric_name",
    "value", "unit", "source_url", "confidence", "notes",
]


# ---- location parsing -------------------------------------------------------

def parse_dpl_zone_locations(source_file: str, zone: str, page_html: str, source_url: str = "") -> list[dict[str, str]]:
...
def parse_dpl_mobile_points_from_html(source_file: str, page_html: str, source_url: str = "") -> list[dict[str, str]]:
    rows: list[dict[str, str]] = []
    seen: set[str] = set()
...
def assign_location_ids(rows: list[dict[str, str]]) -> list[dict[str, str]]:
...
def annual_row_from_text(source_file: str, source_url: str, text: str) -> dict[str, str] | None:
    year = report_year(source_file, text)
    if not year:
        return None
