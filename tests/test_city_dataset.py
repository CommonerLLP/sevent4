...
from sevent4.adapters.filesystem import FileCityConsoleInputRepository
...
class CityDatasetPathTest(unittest.TestCase):
    ...
    def test_city_console_file_input_repository_loads_symlinked_inputs(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            base = Path(tmp)
            repo = base / "repo"
            external = base / "external" / "data" / "cities" / "testville"
            ...
            inputs = FileCityConsoleInputRepository(
                repo_city / "city.yaml",
                repo_city / "layer_manifest.json",
            ).load()

            self.assertEqual("testville", inputs.city.id)
            self.assertEqual(repo_city / "layers", inputs.city.layers_dir)
            self.assertEqual("wards", inputs.manifest.layers[0].id)
